...
def load_data_from_jsonl(filepath):
    if not os.path.exists(filepath):
        print(f"File {filepath} not found. Using default examples.")
        return [
            ...
            {"prompt": "open notepad and write 'hello world'",
             "context": "Opens notepad and writes text.",
             "function": "open_notepad_with_text(text: str)",
             "param_annotations": {"text": ["hello world"]},
...
# ===============================
# Collate Function with Subword Alignment
# ===============================
    ...
    for ex in batch:
        ...
        for param, values in annotations.items():
            if param in param_to_idx:
                ...
                for value in values:
                    subword_ids = tokenizer.encode(value, add_special_tokens=False)
...
# Training Loop
# ===============================
def train_model(model, dataloader, epochs=50, temperature=0.07, output_path="model.pt"):
    model.train()
    ...
    for epoch in range(epochs):
        ...
        for batch in dataloader:
            ...
            param_labels = batch["param_labels"].to(next(model.parameters()).device)
            ...
            prompt_embs = prompt_outputs["sentence_embedding"]
            context_embs = context_outputs["sentence_embedding"]
            param_logits = prompt_outputs["param_logits"]

            contrastive_loss_val = contrastive_loss(prompt_embs, context_embs, temperature)
            param_loss = F.cross_entropy(param_logits.view(-1, param_logits.size(-1)), param_labels.view(-1))
            loss = contrastive_loss_val + param_loss
            loss.backward()
            optimizer.step()
...
# ===============================
test_prompt = "send an email to Alice with subject Project Update and body Please review the report."
extracted = extract_parameters(model, tokenizer, test_prompt, param_to_idx)
